...
drop table SESSION_TYPE cascade constraints PURGE;
...
ALTER TABLE session_type ADD CONSTRAINT session_pk PRIMARY KEY ( session_id );
...
ALTER TABLE qualifying
    ADD CONSTRAINT qualifying_session_fk FOREIGN KEY ( session_id )
        REFERENCES session_type ( session_id );
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'Qualifying' ) THEN
        raise_application_error(-20223,'FK Qualifying_Session_FK in Table Qualifying violates Arc constraint on Table Session_type - discriminator column Session_TYPE doesn''t have value ''Qualifying'''
        );
    END IF;
...
BEGIN
    ...
    IF ( d IS NULL OR d <> 'Race' ) THEN
        raise_application_error(-20223,'FK Race_Session_FK in Table Race violates Arc constraint on Table Session_type - discriminator column Session_TYPE doesn''t have value ''Race'''
        );
    END IF;
